...
function registerCommand() {
  program.command('init [projectName]').description('项目初始化').option('-t, --template <template>', '创建应用模板类型').option('-n, --noverify', '不验证版本').action(function (projectName, options) {
...
function checkNodeVersion() {
  // 1.获取node版本号
  const currentVersion = process.version; // 2. 对比最低版本号
  // 3. semver库做版本号比对

  const lowerVersion = constant.LOWER_NODE_VERSION;

  if (!semver.gte(currentVersion, lowerVersion)) {
    throw new Error(colors.red(`deer-cli 需要安装${lowerVersion}以上版本的Nodejs`));
  }
} // 检查package version
